...
inline void vec3_seta(float (*result) [16], float x, float y, float z)
{
    result[0][0] = x;
    result[1][0] = y;
    result[2][0] = z;
...
inline void vec3_saxpya(float(*result)[16], float alpha, float* v)
{
    result[0][0] += alpha*v[0];
    result[1][0] += alpha*v[1];
    result[2][0] += alpha*v[2];
}
